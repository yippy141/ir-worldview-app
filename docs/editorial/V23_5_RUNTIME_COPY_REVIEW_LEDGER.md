...
Candidate source binding: **Unavailable**
Prepared: 2026-08-24

## 1. Gate decision

This ledger records no editorial acceptance. The runtime-copy candidate is in
an uncommitted working tree, so there is no candidate commit SHA that binds the
generated text to reviewed source. The current `HEAD` must not be substituted
for the uncommitted candidate. Every surface and every generated row therefore
remains `pending`.
...
It is `pending` because a valid row record cannot yet be bound to a candidate
SHA.
...
No row-level records appear below because candidate binding is unavailable.
Once a committed candidate exists, add one record per row and surface context.
