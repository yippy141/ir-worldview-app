...
create table if not exists agg_dimension_buckets (
  ...
  primary key (instrument_version, scoring_version, dimension, bucket)
);
...
create table if not exists agg_completion (
  ...
  count bigint not null default 0 check (count >= 0),
  primary key (instrument_version, tier, step_index)
);
...
create table if not exists agg_item_latency (
  ...
  count bigint not null default 0 check (count >= 0),
  primary key (instrument_version, item_id, latency_bucket_ms)
);
