# Current Case research handoff

Research produces evidence packs; it does not publish directly. Use the actual research-run date
and select consequential cases from the preceding 21 days only after the first day of confused
breaking coverage has passed.

The initial pack should contain:

1. one security or strategic-crisis case;
...
For each candidate, deliver a complete `CurrentCase` record and a short research memo that records:

- 4–8 factual claims and the known uncertainties;
- 3–4 options, each with its logic and accepted tradeoff;
- 3–4 relevant readings, the strongest objection to each, and the evidence that would update it;
- one assumption challenge and internal next-route recommendations;
- primary sources first, followed by authoritative research and high-quality reporting;
...
Store working notes and source audits in this directory. Do not add them to the production catalog.
Before handoff, open every URL, verify dates and institutional authorship, check that each factual
claim has coverage, and record unresolved contradictions rather than smoothing them away.
