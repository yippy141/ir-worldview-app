    ...
    {result ? <div className={styles.episodeGrid}>
      ...
      <div className={access ? accessStyles.decisionForm : styles.verifyForm}>
        ...
        <div className="payoff-dynamic"><fieldset><legend>What is your principal reason?</legend>{episode.reasons.map(reason => <label className={styles.radio} key={reason.id}>
          <input type="radio" autoComplete="off" name={`${episode.id}-reason`} value={reason.id} checked={answer.reason === reason.id} onChange={() => setAnswer({ ...answer, reason: reason.id })} />{reason.text}
        </label>)}</fieldset>
