...
import { worldStageMenuItems } from "@/lib/world-stage/scenes"
...
export function WorldStagePrototype() {
  ...
  function handleMenuKeyDown(event: KeyboardEvent<HTMLButtonElement>, index: number) {
    ...
    if (event.key === "ArrowDown" || event.key === "ArrowRight") {
      nextIndex = (index + 1) % worldStageMenuItems.length
    } else if (event.key === "ArrowUp" || event.key === "ArrowLeft") {
      nextIndex = (index - 1 + worldStageMenuItems.length) % worldStageMenuItems.length
    } else if (event.key === "Home") {
      nextIndex = 0
    } else if (event.key === "End") {
      nextIndex = worldStageMenuItems.length - 1
    }
  ...
  return (
    <main className={styles.stage} id="site-main">
      <WorldStageMap activeScene={activeItem.sceneId} />
