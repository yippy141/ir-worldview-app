...
export function getHybridLabel(result: AiResult): string | null {
  ...
  return `${archetypeLabels[primary[0]]} / ${archetypeLabels[runnerUp[0]]}`
}
...
export function getRunnerUpKey(result: AiResult): AiArchetypeKey {
  const ordered = getOrderedArchetypeEntries(result)
  return ordered[1]?.[0] ?? result.archetypeKey
}

export function getMostDistantArchetypeKey(result: AiResult): AiArchetypeKey {
  const ordered = getOrderedArchetypeEntries(result)
  return ordered[ordered.length - 1]?.[0] ?? result.archetypeKey
}
...
export function buildComparisonCard(result: AiResult): ComparisonCard {
  const ordered = getOrderedArchetypeEntries(result)
  const primaryKey = ordered[0]?.[0] ?? result.archetypeKey
  const runnerUpKey = ordered[1]?.[0] ?? result.archetypeKey
  const farthestKey = ordered[ordered.length - 1]?.[0] ?? result.archetypeKey
...
function getTopContrastAxes(primary: AiArchetypeKey, secondary: AiArchetypeKey): AiAxisKey[] {
...
function sentenceToTitle(text: string): string {
  ...
  if (text.includes("public oversight") || text.includes("sovereignty")) return "State capacity and coordination"
