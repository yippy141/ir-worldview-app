import type { Decision, Episode, EpisodeId } from "@/lib/decision-exercises/content"
...
export function readEpisode(episode: Episode, first: Decision, second: Decision) {
  ...
  if (first.reason === "none" || second.reason === "none") {
    ...
    if (!deferred) {
      ...
      else if (episode.id === "access" && first.option === "enclave" && second.option === "weights") caveat = "The move and stated reasons concern this model release decision. They do not establish an open-source personality or the safety of public release."
